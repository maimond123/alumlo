...
export default function NetworkVisualization({ fullScreen = false }: NetworkVisualizationProps) {
  ...
  useEffect(() => {
    ...
    // Create nodes
    const nodes: Node[] = Array.from({ length: 140}, () => 
      new Node(
    ...
    const animate = () => {
      ...
      // Draw connections
      ctx.strokeStyle = 'rgba(64, 224, 208, 0.45)'
      ctx.lineWidth = 2.0
  ...
  return (
    <div ref={containerRef} className={`fixed ${fullScreen ? 'inset-0' : 'inset-y-0 right-0 w-2/3'} h-full`}>
      <canvas 
        ref={canvasRef}
